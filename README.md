...
On a multi-monitor setup, create `~/.config/omarchy/dino.dock.monitor.json`
...
monitor file, or an empty one, means no preference. This file also
hot-reloads on save. (A legacy `monitor.json` inside the plugin
directory is still read as a fallback, but the new location is
preferred — editing files inside the plugin directory triggers a full
plugin reload and dirties the checkout `omarchy plugin update` pulls
into.)
...
- **Auto-hide (default on)**: the dock slides off-screen when the
  ...
  `~/.config/omarchy/dino.dock.settings.json` containing
  `{"autohide": false}` — hot-reloads on save, same out-of-plugin-dir
...
- **Running indicator**: a small accent-colored dot appears under any
  app with an open window, pinned or not. Matching compares the window's
  Wayland `app_id` against the desktop entry's exact ID, its
  `StartupWMClass`, and — for Chromium-style webapps — the site's domain
  tokens (so the Fastmail webapp matches `Fastmail.desktop`). Matching
  is deliberately exact rather than substring-fuzzy: `code` can never
  claim `code-oss`'s windows. If a dot doesn't light up for some app,
  add `StartupWMClass=<its app_id>` to that app's desktop entry.
...
- `~/.config/omarchy/dino.dock.pinned.json` — your pinned-app list
  ...
  install starts from.
- `~/.config/omarchy/dino.dock.monitor.json` — optional, multi-monitor
  targeting (a legacy in-plugin `monitor.json` is still honored).
- `~/.config/omarchy/dino.dock.settings.json` — optional; currently one
  knob, `{"autohide": false}` to disable auto-hide.
